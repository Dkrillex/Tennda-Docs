---
title: '原生 Gemini 格式'
description: '使用 Google Gemini 原生格式调用 TenndaAI'
...
<ParamField header="Authorization" type="string">
  Bearer Token：`Bearer sk-xxxxxxxxxx`（推荐，与 TenndaAI 其他接口一致）
</ParamField>
...
<Tabs>
  ...
  <Tab title="Node.js">
    ...
    // 若 SDK 支持自定义 baseUrl，设置为 https://api.tennda.ai
...
<Note>
`google-generativeai` 默认请求 Google 官方端点，接入 TenndaAI 时需通过 `client_options` 或环境变量将 `api_endpoint` 设为 `https://api.tennda.ai`。具体以所用 SDK 文档为准。
</Note>
...
**端点**：`POST https://api.tennda.ai/v1beta/models/{model}:embedContent`
...
**端点**：`POST https://api.tennda.ai/v1beta/models/{model}:batchEmbedContents`
