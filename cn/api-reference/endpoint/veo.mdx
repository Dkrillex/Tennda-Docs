...
Veo 是 Google Vertex AI 推出的多模态视频生成模型，支持**文生视频（T2V）**、**首帧约束**与**首尾帧约束**（仅 3.1 系列）生成连贯视频。通过 TenndaAI 统一视频接口调用：先 [提交任务](/cn/api-reference/endpoint/submit-video-task) 获取 `task_id`，再 [查询任务](/cn/api-reference/endpoint/query-video-task) 轮询状态并获取结果。
...
```bash
curl -X GET "https://api.tennda.ai/v1/video/generations/{task_id}" \
  -H "Authorization: Bearer sk-xxxxxxxxxx"
```
