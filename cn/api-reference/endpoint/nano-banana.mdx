---
title: 'Gemini OpenAI 格式（生图）'
description: 'Gemini imagine 系列：OpenAI 兼容 /v1/images/generations 与 /v1/images/edits'
api: 'POST https://api.tennda.ai/v1/images/generations'
---

<Note>
若使用 Gemini 原生 `generateContent` 生图，请参见 [Gemini 原生（图像）](/cn/api-reference/endpoint/gemini-image)。需要 `extra_body.google` 细粒度控制时，也可走 [Gemini OpenAI 格式（对话）](/cn/api-reference/endpoint/gemini-chat-openai) 的 `/v1/chat/completions` 入口（见文末附录）。
</Note>

Gemini imagine 系列（包含 Google 官方"nano banana"等可同时返回**文本 + 图片**的多模态生图模型）通过 TenndaAI，可用 OpenAI 兼容的 `/v1/images/generations` 与 `/v1/images/edits` 接口直接调用。
...
所有接口使用 **Bearer Token** 认证。在 [TenndaAI 控制台](https://client.tennda.ai) 创建令牌后，在请求头中添加：
...
> 平台还会持续接入新的 imagine 系列模型，最新支持列表请在 [TenndaAI 控制台](https://client.tennda.ai) 模型页面查看。
...
* 不同模型 / `quality` 档位的单价请在 [TenndaAI 控制台 - 模型价格](https://client.tennda.ai) 页面查看。
...
5. **保存原始 base64**：拿到 `b64_json` 后立即保存到您自己的存储（OSS / S3），不要重复请求 TenndaAI 拉取——上游不缓存历史结果。
...
切换示例（[chat 入口](/cn/api-reference/endpoint/gemini-chat-openai) 图生图）：
