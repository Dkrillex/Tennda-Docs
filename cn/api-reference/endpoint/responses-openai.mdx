---
title: '原生 OpenAI 格式（Responses）'
api: 'POST https://api.tennda.ai/v1/responses'
---
...
<ParamField body="model" type="string" required>
  模型标识，支持的模型包括：
  - GPT-5 系列：`gpt-5.5`、`gpt-5.4`、`gpt-5.4-pro`、`gpt-5.4-mini`、`gpt-5.4-nano` 等
...
<Tabs>
  <Tab title="简单对话（非流式）">
    ```bash
    curl -X POST "https://api.tennda.ai/v1/responses" \
  ...
  <Tab title="简单对话（流式）">
    ```bash
    curl -N -X POST "https://api.tennda.ai/v1/responses" \
      -H "Content-Type: application/json" \
      -H "Authorization: Bearer sk-xxxxxxxxxx" \
      -d '{
        "model": "gpt-5.2",
