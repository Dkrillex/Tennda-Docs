---
title: 'Gemini OpenAI 格式（对话）'
description: '用 OpenAI 兼容 /v1/chat/completions 调用 Gemini 对话与多模态模型'
api: 'POST https://api.tennda.ai/v1/chat/completions'
---

<Note>
若使用 Google Gemini 原生协议，请参见 [原生 Gemini 格式](/cn/api-reference/endpoint/gemini-native)。通用多模型 Chat Completions 说明见 [原生 OpenAI 格式（ChatCompletions）](/cn/api-reference/endpoint/chat-openai)。
</Note>

> 路径：`POST https://api.tennda.ai/v1/chat/completions`
...
> **优先级**：`extra_body.google.thinking_config` > `reasoning_effort`。传了 `extra_body.google` 后系统自动的思维链适配会关闭，所有 thinking 行为完全由调用方控制。
