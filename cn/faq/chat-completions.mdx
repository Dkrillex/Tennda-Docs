...
TenndaAI 的对话接口完全兼容 OpenAI Chat Completions API 格式，您可以直接使用 OpenAI SDK 或兼容的客户端调用。少数扩展字段可能因模型而异，以具体模型支持为准。
...
2. **检查客户端支持**：如果客户端不展示 `reasoning_content`，可以使用 `tennda_thinking_to_content: true` 将推理内容内联到 `content` 中
