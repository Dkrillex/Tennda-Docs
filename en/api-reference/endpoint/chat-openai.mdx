---
title: 'Native OpenAI Format (ChatCompletions)'
...
<Tabs>
  ...
  <Tab title="Tongyi Qianwen">
    ...
    **Inline reasoning process into content**:
    ...
    If the client does not display `reasoning_content`, you can use `tennda_thinking_to_content: true` to inline reasoning content into `content`:
    ...
    curl -N -X POST "https://api.tennda.ai/v1/chat/completions" \
      ...
      -d '{
        ...
        "stream": true,
        "enable_thinking": true,
        "tennda_thinking_to_content": true,
        "parameters": {
          "incremental_output": true
        },
        "messages": [
...
<Tabs>
  <Tab title="Upload via File URL">
    ...
    client = OpenAI(
        api_key="sk-xxxxxxxxxx",
        base_url="https://api.tennda.ai/v1/responses?api-version=2025-03-01-preview"
    )
