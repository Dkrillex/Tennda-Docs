...
description: 'Call TenndaAI using Google Gemini native format'
...
<ParamField header="Authorization" type="string">
  Bearer token: `Bearer sk-xxxxxxxxxx` (recommended, consistent with other TenndaAI endpoints)
</ParamField>
...
<Tabs>
  <Tab title="cURL (non-streaming)">
    ```bash
    curl -X POST "https://api.tennda.ai/v1beta/models/gemini-2.5-pro:generateContent" \
  ...
  <Tab title="cURL (streaming)">
    ```bash
    curl -N -X POST "https://api.tennda.ai/v1beta/models/gemini-2.5-pro:streamGenerateContent?alt=sse" \
      -H "Content-Type: application/json" \
      -H "Authorization: Bearer sk-xxxxxxxxxx" \
      -d '{
        "contents": [
  ...
  <Tab title="Python (google-generativeai)">
    ...
    genai.configure(
        ...
        transport="rest",
        client_options={"api_endpoint": "https://api.tennda.ai"}
    )
  ...
  <Tab title="Node.js">
    ...
    // If the SDK supports a custom baseUrl, set it to https://api.tennda.ai
...
<Note>
By default, `google-generativeai` calls Google's API. To use TenndaAI, set `api_endpoint` to `https://api.tennda.ai` via `client_options` or environment variables. See your SDK docs for details.
</Note>
...
**Endpoint**: `POST https://api.tennda.ai/v1beta/models/{model}:embedContent`
...
**Endpoint**: `POST https://api.tennda.ai/v1beta/models/{model}:batchEmbedContents`
